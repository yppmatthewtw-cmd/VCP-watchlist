#!/usr/bin/env python3
"""Attach the official 2026-09-03 INTRADAY snapshot (10:37 ET) to the scans.

The upstream repo published a mid-session snapshot on 9/3 but no post-close one,
...
full-day 9/2 official close; the intraday figure is shown in its own column.
"""
...
for f in ("scan_R17_2026-09-03.json", "scan_stage_R10_2026-09-03.json", "scan_PB-R10_2026-09-03.json"):
    scan = json.load(open(f))
    n = 0
    for r in scan["rows"]:
        hit = INTRA.get(ALIAS.get(r["ticker"], r["ticker"]))
        if hit:
            r["px_0903_intraday"], r["chg_0903_intraday"] = round(hit[0], 2), round(hit[1], 2)
            n += 1
    json.dump(scan, open(f, "w"), ensure_ascii=False, indent=1)
    print(f"{f}: 9/3 intraday attached to {n}/{len(scan['rows'])}")

vals = [r["chg_0903_intraday"] for r in json.load(open("scan_R17_2026-09-03.json"))["rows"]
        if r.get("chg_0903_intraday") is not None]
import statistics
print(f"universe 9/3 intraday median {statistics.median(vals):+.2f}% | "
