#!/usr/bin/env python3
"""R15 snapshot refresh: 2026-09-08 closes (Yahoo, cross-checked against the official series) for the whole universe, with
the classification fixes from the 2026-09-02 critical review.

Sources: cert7_2026-09-08.json (official series, 241 tickers) + the 9/8 HEAD (mid-session) snapshot for market cap / sector only
...
R19 -> R20, stage R12 -> R13, PB R12 -> R13. Old snapshots are kept untouched.
"""
...
def apply_official(r):
    ...
    for k in ("px_0904_intraday", "chg_0904_intraday"):
        r.pop(k, None)
    r["as_of"] = AS_OF if src == "series" else "2026-09-04"   # HEAD fallback = 9/4 close (mid-session 9/8 snapshot)
    r.pop("stale", None)
...
JOBS = [("scan_R19_2026-09-06.json", "scan_R20_2026-09-09.json", "vcp"),
