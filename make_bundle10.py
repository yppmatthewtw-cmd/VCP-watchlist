...
CERT = json.load(open("cert7_2026-09-14.json"))
...
def intra_cell(r):
    """R19: no mid-session column — the 9/14 close (Yahoo, cross-checked) is the newest data."""
    return ""
...
<p class="lede">{list_name}清單內{tier_name}（依 9/14 收盤市值），先按級別（{"A → B → E" if list_no != "2" else "2A → 2B → 1→2"}）、再按該清單分數由高至低{cap_note}{miss_note}。
...
<th>催化</th><th class="num sort">收盤<small>9/14 Yahoo</small></th><th class="num sort">距高</th>{head_extra}
...
<s class="old">A</s><b class="upd">B</b>＝等級變動 · <small class="rk up">▲3</small>／<small class="rk dn">▼3</small>＝名次升降 · <small class="day dn">-1.2%</small>＝9/14 當日漲跌</span></div>
...
<div class="urow"><b>9/14 當日</b><span class="utxt">最強：{esc(up) or "–"}｜最弱：{esc(dn) or "–"}</span></div><div class="urow"><b>數據驗證</b><span class="utxt">本版分級規則與 R18 相同，等級變動全部來自價格與財報。<b>R18 以 Yahoo 單一來源發布的 9/11 收盤，已被 9/14 官方中場快照的 price_change 獨立驗證：240 檔、中位偏差 0.000%、無一檔偏差 &gt;0.1%</b>；本版 9/14 收盤同樣來自 Yahoo，官方序列已延伸至 9/11。<b>另注</b>：本版的官方序列視窗由 2026-03-31 起延長至 <b>2025-12-26 起（179 個交易日）</b>（研究環境重建後重新完整複製上游倉庫所致），確定性的「底部偵測／回檔收縮」與市場百分位參考集因而略有變動；分級規則本身不變。</span></div></div>"""
...
<th class="num sort">收盤<small>9/14 Yahoo</small></th><th class="num sort">距高</th><th class="num sort">市值</th>
...
def write_excel(path, cap_defs, rec, rev, model, stamp_txt, basis_txt):
    ...
    def cap_sheet(ws, snaps, tiers, online, extra_cols, pred):
        ...
        prev_items, _ = top50(PREV_ROWS.get(pkey, []), letter_of, online, pred)
        prev_rank = {r["ticker"]: i for i, r in enumerate(prev_items, 1)}
        cols = (["排名", "前版名次", "代號", "名稱", "產業", "等級", "前版等級", "線上", "催化", "收盤", "基準日", "9/14當日%", "低於高點%"]
        ...
        for i, r in enumerate(items, 1):
            ...
            if old_letter and old_letter != letter:      # grade change: red grade cell
                c.font = upd_font
            for col, key, tol in ((10, "price", 0.005), (15, "off_high_pct", 0.05), (len(cols) - 3, "score", 0.05)):
        ...
        style_sheet(ws, len(cols), [6, 8, 9, 26, 16, 7, 8, 7, 30, 10, 11, 9, 10, 10, 10] + [9] * len(extra_cols)
                    + [10, 9, 9, 8, 8, 10, 50])
    ...
    ws4.append(["代號", "名稱", "產業", "交易所", "上升分數", "前版上升分數", "催化", "主因", "VCP", "前版VCP",
                "Weinstein", "前版Wein", "Pre-breakout", "前版PB", "線上數", "收盤", "9/14當日%", "低於高點%", "市值($B)",
                "級距", "確定性總分", "前版確定性"]
    ...
    for e in sorted(rec.values(), key=lambda e: (-e.get("rise_score", 0), -e["online_count"], -e["best_score"])):
        ...
        if c and changed(c["cert"], pc, 3):
            ws4.cell(row=row, column=23).font = upd_font
        if cat and (PREV_CAT.get(t) or {}).get("reason") != cat["reason"]:
            ws4.cell(row=row, column=7).font = upd_font
        elif cat:
            ws4.cell(row=row, column=7).font = hot_font if cat["pts"] > 0 else warn_font
        lc = ws4.cell(row=row, column=24 + len(C7_COLS) + 2)
        lc.hyperlink = tv_url(t)
        lc.value = "chart"
        lc.font = link_font
        ws4.cell(row=row, column=1).font = Font(bold=True)
    style_sheet(ws4, 24 + len(C7_COLS) + 2,
                [9, 26, 16, 9, 9, 9, 28, 30, 7, 7, 10, 8, 12, 8, 8, 10, 9, 10, 10, 9, 10, 6, 10, 9] + [8] * len(C7_COLS) + [8, 10])
...
def main():
    ...
    ap.add_argument("--prev", default="scan_R23_2026-09-12.json,scan_stage_R16_2026-09-12.json,scan_PB-R16_2026-09-12.json",
                    help="previous release's three snapshot files (vcp,stage,pre) for the red diff")
    ap.add_argument("--prev-rank", default="rank_overlay_R18.json")
    ap.add_argument("--prev-cert", default="cert7_R18.json")
    ap.add_argument("--prev-cat", default="catalysts_R18.json")
    args = ap.parse_args()
    ...
    # macro line from the rows themselves (sector medians of the 9/8 move)
    import statistics
    global MACRO
    by_sec, allmv = {}, []
    ...
    XC = json.load(open("yahoo_crosscheck_R19.json"))
    V4 = json.load(open("validate_r18_0911.json"))
    MS = json.load(open(sorted(glob.glob("market_scan_*.json"))[-1]))
    n_yonly = sum(1 for sn in (vcp_snaps, stage_snaps, pre_snaps) for r in sn[-1][3]["rows"] if r.get("px_src") == "yahoo")
    ...
    IDX = ("AI 與晶片股急挫拖累大市：週末 Anthropic 行政總裁 Dario Amodei 公開呼籲放慢 AI 發展步伐以控制風險，"
           "OpenAI 的 Altman 與 xAI 的 Musk 表態認同，VanEck 半導體 ETF 單日挫逾 4%；"
           "同時油價續升、中東局勢再緊張，10 年期美債殖利率升至 4.97%（2023 年以來最高）："
           "納指 −0.99% 收 26,072.42（−261 點）、標普 −0.37% 收約 7,629、道指 −0.28%。"
           "本週 9/15–16 FOMC，市場預期加息 25 個基點。")
    MACRO = (f"宏觀：<b>9/14 收盤（本表分級基準）</b>— {IDX}"
             f"本清單 {len(allmv)} 檔中位數 {statistics.median(allmv):+.2f}%，板塊最弱 {weakest}；最強 {strongest}。"
             f"下一焦點：<b>9/15–16 FOMC 議息結果（明晚）</b>。"
             f"<b>數據</b>：上游快照倉庫仍只在美東上午發布中場快照（9/14 為 12:24 ET），9/14 收盤與成交量取自 <b>Yahoo Finance 日線</b>"
             f"（GitHub Actions runner 以 yfinance 拉取），並與官方序列逐日交叉核對"
             f"（{XC['n_symbols']:,} 檔 × 全窗 {XC['n_pairs']:,} 個收盤，中位偏差 {XC['median_pct']:.3f}%）；"
             f"<b>上版（R18）的 9/11 收盤已被 9/14 官方快照獨立驗證：240 檔中位偏差 0.000%，<u>無一檔偏差 &gt;0.1%</u></b>")
    ...
    xc_days = "、".join(d[5:].replace("-", "/").lstrip("0").replace("/0", "/") for d in XC.get("replaced_days", []))
    doc = HTML_SHELL.format(
        xc_symbols=f"{XC['n_symbols']:,}", xc_pairs=f"{XC['n_pairs']:,}", xc_median=f"{XC['median_pct']:.3f}",
        xc_gt=f"{XC['gt_0_5pct']:,}", xc_gt_pct=f"{XC['gt_0_5pct'] / max(XC['n_pairs'], 1) * 100:.2f}",
        xc_splits=len(XC["split_adj"]), xc_days=xc_days,
        v4_n=V4["n"], v4_med=f'{V4["median_pct"]:.3f}', v4_gt=V4["gt_0_5pct"],
        v4_gt_pct=f'{V4["gt_0_5pct"] / max(V4["n"], 1) * 100:.1f}',
        ms_elig=f'{MS["eligible"]:,}', ms_2b=f'{MS["coverage"]["eligible_ge_2b"]:,}',
        ms_cov=f'{MS["coverage"]["pct_of_ge_2b"]:.0f}',
        ms_a=MS["VCP_A"]["total"], ms_a_in=MS["VCP_A"]["in_universe"], ms_a_out=MS["VCP_A"]["outside"],
        ms_2a=MS["Stage_2A"]["total"], ms_2a_in=MS["Stage_2A"]["in_universe"],
        ms_2a_out=MS["Stage_2A"]["outside"],
        ms_names=esc("、".join(f"{r['t']}（${r['mcap_b']:.0f}B）" for r in MS["VCP_A"]["top_outside"]
                              if not r["pegged"] and r["mcap_b"] >= 2)[:200]),
        ms_names2=esc("、".join(f"{r['t']}（${r['mcap_b']:.0f}B）" for r in MS["Stage_2A"]["top_outside"]
                               if not r["pegged"] and r["mcap_b"] >= 2)[:200]),
...
<li><b>9/14 收盤的來源（請務必留意）</b>：上游快照倉庫（zyhe16/top-us-stock-tickers）自 9/2 起只在美東上午發布一次中場快照，
9/14 亦然（16:24 UTC ＝ 12:24 ET），因此<b>9/14 收盤與成交量取自 Yahoo Finance 日線</b>（本倉庫的 GitHub Actions runner 以 yfinance 拉取後提交，
研究容器的 egress proxy 封鎖 Yahoo）。9/11 及之前每一天都與官方序列逐檔逐日交叉核對：{xc_symbols} 檔、{xc_pairs} 個收盤，
中位偏差 {xc_median}%，偏差 &gt;0.5% 的僅 {xc_gt} 個（{xc_gt_pct}%）。</li>
<li><b>上一版 Yahoo 收盤已獲官方驗證（每版例行檢查）</b>：R18 的 9/11 收盤是 Yahoo 單一來源、當時沒有官方數據可對照。
9/14 的中場快照發布後，其 <code>price_change</code> 欄正好還原出<b>官方 9/11 收盤</b>。
兩者逐檔比對：<b>{v4_n} 檔，中位偏差 {v4_med}%，偏差 &gt;0.5% 的 {v4_gt} 檔（{v4_gt_pct}%）</b> ——
本輪<b>無一檔偏差超過 0.1%</b>，是五輪檢查以來最乾淨的一次。Yahoo 作為第二來源<b>連續五版通過驗證</b>：R14 的 9/4 為 2 檔 &gt;0.5%、R15 的 9/8 為 0 檔、R16 的 9/9 僅 HUBB（該檔當時已標示為較舊基準）、R17 的 9/10 為 1 檔（UBSI 0.81%）、本輪 R18 的 9/11 為 0 檔。本版的 9/14 收盤同樣要等下一個中場快照（預計 9/15）才能作同一驗證。</li>
<li><b>Yahoo 補齊的「沒有數據的部份」</b>：(a) 33 檔序列不完整的 ADR／外國股（ARM、ASML、TSM、NBIS、IREN、CLS、FN、MNDY 等）本版改用 Yahoo 完整日線計算全部指標，不再靠中場快照反推；
(b) 官方序列以 price_change 反推的日子（8/27、8/31、9/1–9/3）其近似成交量改為 Yahoo 全日實際成交量（影響量縮分項）；
交叉核對亦揭露上游漏發快照、擷取時以前值補位的 {xc_days} 三天在多數股票上與 Yahoo 不符，本版整天改用 Yahoo 實際收盤與成交量；
(c) <b>MA200、52 週高低點、6 個月與 1 年動能改以 Yahoo 250 日歷史（2025-09-01 起）重算</b>，取代原掃描沿用的舊值與 5 個月代理（自 R14 起）。</li>
...
<li><b>官方序列視窗延長（本版變動）</b>：研究環境於 9/12–9/14 間重建，暫存的序列檔與上游倉庫複本一併清空；重新複製上游倉庫後，
可用的快照歷史由原本的 2026-03-31 起<b>延長至 2025-12-26 起（179 個交易日）</b>。確定性 7 項中的「底部序列／回檔收縮」與
市場百分位參考集（rs／量比／收縮）因而在較長視窗上重算，<b>部分確定性分數與等級變動源於此，而非 9/14 價格</b>；
分級規則與 R18 完全相同。MA200／52 週高低／6 個月／1 年動能自 R14 起已改用 Yahoo 250 日歷史，不受此變動影響。
較長視窗亦令交叉核對的樣本由 33 萬個收盤增至 52 萬個：中位偏差仍為 0.000%，偏差 &gt;0.5% 佔 0.31%（集中在 MIDD、SCCO、GAB、JRI 等少數有除息調整差異的股票，近兩週每日皆 ≤15 檔）。</li>
<li><b>數據基準</b>：本版為<b>完整的 2026-09-14 收盤重掃</b> —— 官方日線序列已延伸至 9/11，
其後一個交易日（9/14）由 Yahoo 補上；33 檔序列有缺口的 ADR／外國股全程使用 Yahoo。市值按 9/14 收盤重算
（上游市值以 9/14 中場價計，已依收盤／中場價比例換算）。1 月／3 月／5 日動能、MA50 自合併序列計算；
MA200、52 週高低點、6 個月／1 年動能以 Yahoo 250 日歷史計算。</li>
<li><b>本版批判性檢視：清單宇宙的覆蓋率（重要）</b>：本表的 274 檔宇宙是最初建表時掃描一次後沿用至今。
本版首次用<b>同一套分級規則</b>掃描整個流動性達標的美股市場（{ms_elig} 檔有 250 日歷史、股價 ≥$2、20 日中位成交額 ≥$1M），
結果：<b>A 級（VCP 待突破）全市場 {ms_a} 檔，其中只有 {ms_a_in} 檔在本清單內，{ms_a_out} 檔在清單以外</b>；
<b>2A（初升段）全市場 {ms_2a} 檔，清單內 {ms_2a_in} 檔，清單外 {ms_2a_out} 檔</b>。
以市值 ≥$2B 計，本清單只覆蓋全市場合資格股的 <b>{ms_cov}%</b>。
換言之，各頁的「TOP 50」是<b>這 274 檔之內的前 50</b>，不是全市場前 50 —— 這是目前最大的結構性限制。
清單外分數最高、且非併購釘價的中大型 A 級例子：{ms_names}；2A 例子：{ms_names2}。
（部分高分股其實是<b>現金收購釘價</b>造成的假收縮，已在掃描中標記剔除。）
如需把宇宙擴大到全市場，請告知 —— 資料管線已具備條件，但屆時與前版的逐檔比較會大幅斷裂。</li>
<li><b>拆股調整</b>：CRWD 4:1（7/02）、KLAC 10:1（6/12）、DD 1:3 反向（6/24）及 <b>RUSHB／RUSHA 3:2（8/31 配發，已核對 8-K）</b>
均已調整；RUSHB 原掃描的 52 週高低點與均線已按 2/3 換算。確定性 7 項量化以延伸至 9/14 的合併序列重算（274/274 檔有完整序列，
49 檔具一底高於一底結構）。</li>
<li><b>紫色標示＝相對前一版的變動</b>（刻意不用紅色，以免與漲跌顏色混淆）：每頁頂部的「本版更新」框列出新進榜／跌出榜／等級升降／
當日最強最弱；表內紫字為數值已變（滑鼠停留顯示前版值）、<b>NEW</b> 為新進榜、劃線舊等級→紫色新等級為等級變動、
▲▼ 為名次升降、價格下方的小字為 9/14 當日漲跌。確定性分項 |Δ|≥3 才標示，以免全表泛紅。</li>
...
現以官方序列全長（約 5 個月）補上，2A 由 8 檔回到 29 檔、2B 由 11 檔回到 26 檔。(c) 缺 1 年數據時，
改以「距 52 週低點漲逾 150%」判斷是否已延伸。</li>
<li><b>上一版批判性檢視後的修訂</b>：(1) VCP — 21 日漲幅 &gt;15% 一律歸 E（延伸），不再被當作 A/B/C 基底；A 級另需 21 日收盤區間 ≤12%；
