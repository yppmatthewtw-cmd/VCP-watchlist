...
def intra_cell(r):
    """R21: no mid-session column — the 9/16 close (Yahoo, cross-checked) is the newest data."""
    return ""
...
<p class="lede">{list_name}清單內{tier_name}（依 9/16 收盤市值），先按級別（{"A → B → E" if list_no != "2" else "2A → 2B → 1→2"}）、再按該清單分數由高至低{cap_note}{miss_note}。
...
<th>催化</th><th class="num sort">收盤<small>9/16 Yahoo</small></th><th class="num sort">距高</th>{head_extra}
...
<s class="old">A</s><b class="upd">B</b>＝等級變動 · <small class="rk up">▲3</small>／<small class="rk dn">▼3</small>＝名次升降 · <small class="day dn">-1.2%</small>＝9/16 當日漲跌</span></div>
...
<div class="urow"><b>9/16 當日</b><span class="utxt">最強：{esc(up) or "–"}｜最弱：{esc(dn) or "–"}</span></div><div class="urow"><b>數據驗證</b><span class="utxt">本版分級規則與 R20 相同，等級變動全部來自價格與財報。<b>R20 以 Yahoo 單一來源發布的 9/15 收盤，已被 9/16 官方中場快照的 price_change 獨立驗證：240 檔、中位偏差 0.000%、無一檔偏差 &gt;0.1%</b>；本版 9/16 收盤同樣來自 Yahoo，官方序列已延伸至 9/15。</span></div></div>"""
...
<th class="num sort">收盤<small>9/16 Yahoo</small></th><th class="num sort">距高</th><th class="num sort">市值</th>
...
def write_excel(path, cap_defs, rec, rev, model, stamp_txt, basis_txt):
    ...
    def cap_sheet(ws, snaps, tiers, online, extra_cols, pred):
        ...
        prev_items, _ = top50(PREV_ROWS.get(pkey, []), letter_of, online, pred)
        prev_rank = {r["ticker"]: i for i, r in enumerate(prev_items, 1)}
        cols = (["排名", "前版名次", "代號", "名稱", "產業", "等級", "前版等級", "線上", "催化", "收盤", "基準日", "9/16當日%", "低於高點%"]
    ...
    ws4.append(["代號", "名稱", "產業", "交易所", "上升分數", "前版上升分數", "催化", "主因", "VCP", "前版VCP",
                "Weinstein", "前版Wein", "Pre-breakout", "前版PB", "線上數", "收盤", "9/16當日%", "低於高點%", "市值($B)",
                "級距", "確定性總分", "前版確定性"]
...
def main():
    ...
    ap.add_argument("--prev", default="scan_R25_2026-09-16.json,scan_stage_R18_2026-09-16.json,scan_PB-R18_2026-09-16.json",
                    help="previous release's three snapshot files (vcp,stage,pre) for the red diff")
    ap.add_argument("--prev-rank", default="rank_overlay_R20.json")
    ap.add_argument("--prev-cert", default="cert7_R20.json")
    ap.add_argument("--prev-cat", default="catalysts_R20.json")
    args = ap.parse_args()
    ...
    XC = json.load(open("yahoo_crosscheck_R21.json"))
    V4 = json.load(open("validate_r20_0915.json"))
    ...
    IDX = ("<b>聯儲局三年來首次加息</b>：一致決定上調聯邦基金利率 25 個基點至 <b>3.75–4.00%</b>，"
           "主席 Warsh 於記者會強調通脹仍然過高、承諾「更及時地」回到 2%，並暗示年內或再加一次；"
           "10 年期美債殖利率再度升穿 5%。指數在決議公布後轉弱："
           "道指 −1.21% 收 51,461.90（−631 點）、標普 −0.45% 收 7,551.81、納指 −0.01% 收 25,978.42（近乎持平）。")
...
<li><b>9/16 收盤的來源（請務必留意）</b>：上游快照倉庫（zyhe16/top-us-stock-tickers）自 9/2 起只在美東上午發布一次中場快照，
9/16 亦然（15:00 UTC ＝ 11:00 ET），因此<b>9/16 收盤與成交量取自 Yahoo Finance 日線</b>（本倉庫的 GitHub Actions runner 以 yfinance 拉取後提交，
研究容器的 egress proxy 封鎖 Yahoo）。9/15 及之前每一天都與官方序列逐檔逐日交叉核對：{xc_symbols} 檔、{xc_pairs} 個收盤，
中位偏差 {xc_median}%，偏差 &gt;0.5% 的僅 {xc_gt} 個（{xc_gt_pct}%）。</li>
<li><b>上一版 Yahoo 收盤已獲官方驗證（每版例行檢查）</b>：R20 的 9/15 收盤是 Yahoo 單一來源、當時沒有官方數據可對照。
9/16 的中場快照發布後，其 <code>price_change</code> 欄正好還原出<b>官方 9/15 收盤</b>。
兩者逐檔比對：<b>{v4_n} 檔，中位偏差 {v4_med}%，偏差 &gt;0.5% 的 {v4_gt} 檔（{v4_gt_pct}%）</b> ——
本輪<b>無一檔偏差超過 0.1%</b>。Yahoo 作為第二來源<b>連續七版通過驗證</b>（R14→R20 每版偏差 &gt;0.5% 的檔數：2、0、1、1、0、1、0）。本版的 9/16 收盤同樣要等下一個中場快照（預計 9/17）才能作同一驗證。</li>
...
<li><b>數據基準</b>：本版為<b>完整的 2026-09-16 收盤重掃</b> —— 官方日線序列已延伸至 9/15，
其後一個交易日（9/16）由 Yahoo 補上；33 檔序列有缺口的 ADR／外國股全程使用 Yahoo。市值按 9/16 收盤重算
（上游市值以 9/16 中場價計，已依收盤／中場價比例換算）。1 月／3 月／5 日動能、MA50 自合併序列計算；
...
均已調整；RUSHB 原掃描的 52 週高低點與均線已按 2/3 換算。確定性 7 項量化以延伸至 9/16 的合併序列重算（274/274 檔有完整序列，
49 檔具一底高於一底結構）。</li>
<li><b>紫色標示＝相對前一版的變動</b>（刻意不用紅色，以免與漲跌顏色混淆）：每頁頂部的「本版更新」框列出新進榜／跌出榜／等級升降／
當日最強最弱；表內紫字為數值已變（滑鼠停留顯示前版值）、<b>NEW</b> 為新進榜、劃線舊等級→紫色新等級為等級變動、
▲▼ 為名次升降、價格下方的小字為 9/16 當日漲跌。確定性分項 |Δ|≥3 才標示，以免全表泛紅。</li>
