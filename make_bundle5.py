...
CERT = json.load(open("cert7_2026-09-04.json"))
...
def intra_cell(r):
    """R14: no mid-session column — the 9/4 close (Yahoo, cross-checked) is the newest data."""
    return ""
...
<p class="lede">{list_name}清單內{tier_name}（依 9/4 收盤市值），先按級別（{"A → B → E" if list_no != "2" else "2A → 2B → 1→2"}）、再按該清單分數由高至低{cap_note}{miss_note}。
...
<th>催化</th><th class="num sort">收盤<small>9/4 Yahoo</small></th><th class="num sort">距高</th>{head_extra}
...
<s class="old">A</s><b class="upd">B</b>＝等級變動 · <small class="rk up">▲3</small>／<small class="rk dn">▼3</small>＝名次升降 · <small class="day dn">-1.2%</small>＝9/4 當日漲跌</span></div>
...
<div class="urow"><b>9/4 當日</b><span class="utxt">最強：{esc(up) or "–"}｜最弱：{esc(dn) or "–"}</span></div><div class="urow"><b>數據修正</b><span class="utxt">本版分級規則不變；但 <b>MA200、52 週高低點、6 個月／1 年動能改以 Yahoo 250 日歷史重算</b>（前版沿用原掃描舊值或 5 個月代理），33 檔 ADR 亦改用 Yahoo 完整序列 — 因此部分等級變動源於數據修正而非 9/4 價格；9/4 收盤本身來自 Yahoo（官方快照未發布），已與官方序列交叉核對。</span></div></div>"""
...
<th class="num sort">收盤<small>9/4 Yahoo</small></th><th class="num sort">距高</th><th class="num sort">市值</th>
...
def write_excel(path, cap_defs, rec, rev, model, stamp_txt, basis_txt):
    ...
    def cap_sheet(ws, snaps, tiers, online, extra_cols, pred):
        ...
        prev_items, _ = top50(PREV_ROWS.get(pkey, []), letter_of, online, pred)
        prev_rank = {r["ticker"]: i for i, r in enumerate(prev_items, 1)}
        cols = (["排名", "前版名次", "代號", "名稱", "產業", "等級", "前版等級", "線上", "催化", "收盤", "基準日", "9/4當日%", "低於高點%"]
    ...
    ws4.append(["代號", "名稱", "產業", "交易所", "上升分數", "前版上升分數", "催化", "主因", "VCP", "前版VCP",
                "Weinstein", "前版Wein", "Pre-breakout", "前版PB", "線上數", "收盤", "9/4當日%", "低於高點%", "市值($B)",
                "級距", "確定性總分", "前版確定性"]
...
def main():
    ...
    ap.add_argument("--prev", default="scan_R18_2026-09-05.json,scan_stage_R11_2026-09-05.json,scan_PB-R11_2026-09-05.json",
                    help="previous release's three snapshot files (vcp,stage,pre) for the red diff")
    ap.add_argument("--prev-rank", default="rank_overlay_R13.json")
    ap.add_argument("--prev-cert", default="cert7_R13.json")
    ap.add_argument("--prev-cat", default="catalysts_R13.json")
    args = ap.parse_args()
    ...
    XC = json.load(open("yahoo_crosscheck_R14.json"))
    ...
    MACRO = (f"宏觀：<b>9/4 收盤（本表分級基準）</b>— 8 月非農 +16.2 萬（預期 5.5 萬）令 9 月<b>升息</b>機率升至五至六成、10 年期殖利率 4.79%；"
             f"標普 −0.38% 收 7,718.60、納指 −0.29% 收 26,506.99、道指 −272 點收 53,414.25，惟半導體設備／AI 互連逆市大漲（KLAC +7.3%、LRCX +5.1%、ALAB +10%），"
             f"LULU 財報砍指引 −17%、TSLA Cybercab 發表後 −6%；本清單 {len(allmv)} 檔中位數 {statistics.median(allmv):+.2f}%，"
             f"板塊最弱 {weakest}；最強 {strongest}。9/7 勞動節休市，下一焦點 9/11 CPI 與 9/15–16 FOMC。"
             f"<b>數據</b>：上游快照倉庫仍未發布 9/4 收盤，本版 9/4 收盤與成交量全部取自 <b>Yahoo Finance 日線</b>（GitHub Actions runner 以 yfinance 拉取），"
             f"並與官方序列逐日交叉核對（{XC['n_symbols']:,} 檔 × 全窗 {XC['n_pairs']:,} 個收盤，中位偏差 {XC['median_pct']:.3f}%）；"
             f"MA200／52 週高低／6 月／1 年動能亦改以 Yahoo 250 日歷史重算")
...
<li><b>9/4 收盤的來源（請務必留意）</b>：上游快照倉庫（zyhe16/top-us-stock-tickers）自 9/2 起只在美東上午發布一次中場快照，
截至本表產生時仍未發布 9/4 收盤快照。本版採用與 10MA Watchlist 相同的做法：研究容器無法連接 Yahoo，故由本倉庫的 GitHub Actions runner 執行
<code>scripts/fetch_yahoo.py</code>（yfinance，未除息調整價）拉取 <code>data/yahoo/tickers.txt</code> 內 3,052 檔（274 檔清單＋市場百分位參考集）
2025-09-01 → 2026-09-04 的日線並提交至 <code>data/yahoo/</code>，再由 <code>merge_yahoo.py</code> 併入官方序列。<b>9/4 收盤與成交量全部來自 Yahoo</b>（單一來源，尚無官方快照可對照）；
9/3 及之前每一天都與官方序列逐檔逐日交叉核對：{xc_symbols} 檔、{xc_pairs} 個收盤，中位偏差 {xc_median}%，
偏差 &gt;0.5% 的僅 {xc_gt} 個（{xc_gt_pct}%，集中在少數股票的補值日）；
9/2、9/3 兩天由 price_change 反推的官方收盤與 Yahoo 100% 吻合。Yahoo 的收盤價已按拆股調整，官方序列則為原始成交價，
合併時以兩者比值把官方序列移到拆股後基準（本次偵測到 {xc_splits} 檔，含 CRWD 4:1、KLAC 10:1、DD 1:3、RUSHB／RUSHA 3:2、APH 2:1，與先前人工核對一致）。</li>
...
(c) <b>MA200、52 週高低點、6 個月與 1 年動能改以 Yahoo 250 日歷史（2025-09-01 起）重算</b>，取代原掃描沿用的舊值與 5 個月代理 —— 這是本版等級變動偏多的主因（部分等級變動源於數據修正而非 9/4 價格）。</li>
...
<li><b>數據基準</b>：本版為<b>完整的 2026-09-04 收盤重掃</b> —— 274 檔全部取得 9/4 收盤：241 檔為官方序列（至 9/3）＋Yahoo 9/4，33 檔為 Yahoo 完整序列；
市值按 9/4 收盤重算（上游市值以 9/4 中場價計，已依收盤／中場價比例換算）。1 月／3 月／5 日動能、MA50 自合併序列計算。</li>
<li><b>拆股調整</b>：CRWD 4:1（7/02）、KLAC 10:1（6/12）、DD 1:3 反向（6/24）及 <b>RUSHB／RUSHA 3:2（8/31 配發，已核對 8-K）</b>
均已調整；RUSHB 原掃描的 52 週高低點與均線已按 2/3 換算。確定性 7 項量化以延伸至 9/4 的合併序列重算（274/274 檔有完整序列，
49 檔具一底高於一底結構）。</li>
<li><b>紫色標示＝相對前一版的變動</b>（刻意不用紅色，以免與漲跌顏色混淆）：每頁頂部的「本版更新」框列出新進榜／跌出榜／等級升降／
當日最強最弱；表內紫字為數值已變（滑鼠停留顯示前版值）、<b>NEW</b> 為新進榜、劃線舊等級→紫色新等級為等級變動、
▲▼ 為名次升降、價格下方的小字為 9/4 當日漲跌。確定性分項 |Δ|≥3 才標示，以免全表泛紅。</li>
