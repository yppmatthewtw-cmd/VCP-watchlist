...
def hotbar(tickers):
    ...
    return (f'<div class="hotbar"><span class="hlabel">🔥 本週熱點催化</span>'
            f'<span class="hmacro">宏觀（9/1）：美軍再度空襲伊朗革命衛隊目標（荷姆茲海峽），油價與債息同步走高；<b>科技股領跌</b>，納指 -1.02%（XLK -1.6%）、道指 -419 點，公用事業續弱；能源與防禦性醫藥逆勢走強</span>'
            f'{chips}</div>')
...
因此 8/31 與 9/1 的收盤價都只能逐檔以網絡搜尋查證：8/31 取得 67/274 檔，<b>9/1 僅再取得 9 檔</b>
（同日收盤的搜尋結果極為稀少）。本表因此同時存在三個基準日 —— 收盤價下方以紅色小字標示該列的實際
基準日（<b>無標示者即為 9/1</b>，標 08/31 或 08/28 者為較早收盤）。<b>本版的主要更新在於宏觀與催化
層面（9/1 科技領跌、油價與債息走高），而非全巿場價格重掃</b>；跨檔比較分數時請務必以基準日標示為準。</li>
