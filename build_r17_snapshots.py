#!/usr/bin/env python3
"""R17 snapshot refresh: 2026-09-10 closes (Yahoo, cross-checked against the official series) for the whole universe, with
the classification fixes from the 2026-09-02 critical review.

Sources: cert7_2026-09-10.json (official series, 241 tickers) + the 9/10 HEAD (mid-session) snapshot for market cap / sector only
...
R21 -> R22, stage R14 -> R15, PB R14 -> R15. Old snapshots are kept untouched.
"""
...
def apply_official(r):
    ...
    for k in ("px_0904_intraday", "chg_0904_intraday"):
        r.pop(k, None)
    r["as_of"] = AS_OF if src == "series" else "2026-09-09"   # HEAD fallback = 9/9 close (mid-session 9/10 snapshot)
    r.pop("stale", None)
...
JOBS = [("scan_R21_2026-09-10.json", "scan_R22_2026-09-11.json", "vcp"),
        ("scan_stage_R14_2026-09-10.json", "scan_stage_R15_2026-09-11.json", "stage"),
        ("scan_PB-R14_2026-09-10.json", "scan_PB-R15_2026-09-11.json", "pb")]
