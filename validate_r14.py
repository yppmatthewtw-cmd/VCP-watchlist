#!/usr/bin/env python3
"""Check R14's Yahoo-sourced 2026-09-04 closes against the official close.

R14 published 9/4 from Yahoo alone — the upstream snapshot repo had nothing
newer than a 9/4 mid-session quote. The 9/8 mid-session snapshot's
...
assert CAL[-1] == "2026-09-04", CAL[-1]
filled_94 = set(FILLED.get("2026-09-04", []))
...
for t, r in pub.items():
    s = SER.get(t)
    if not s or s[0] + len(s[1]) != len(CAL) or t in filled_94:
        missing.append(t)
        continue
    official = s[1][-1]
    px = r["price"]
...
json.dump(out, open("validate_r14_0904.json", "w"), ensure_ascii=False, indent=1)
print(f"R14 9/4 published vs official: n={out['n']} median |Δ|={out['median_pct']}% "
