#!/usr/bin/env python3
"""R3 data build: official 8/28 closes + 7-item certainty + market-cap tiers.

Sources: zyhe16/top-us-stock-tickers daily snapshots (rebuilt into series2.pkl)
...
pickle.dump({"cal": CAL, "series": SER}, open(f"{SCRATCH}/series2adj.pkl", "wb"))
...
MY = set()
for f in ("scan_R12_2026-08-29.json", "scan_stage_R5_2026-08-29.json", "scan_PB-R5_2026-08-29.json"):
    MY |= {r["ticker"] for r in json.load(open(f))["rows"]}
...
rets = {s: SER[s][1][-1] / SER[s][1][-22] - 1 for s in have if len(SER[s][1]) >= 22}
...
cs_ = list(contr_vals)
for s, p in zip(cs_, pct_ranks([contr_vals[s] for s in cs_])):
    struct[s]["s_contr"] = 1 - p
rs_syms = list(univ)
for s, a, b in zip(rs_syms, pct_ranks([univ[s]["rs21"] for s in rs_syms]),
                   pct_ranks([univ[s]["dv_ratio"] for s in rs_syms])):
    univ[s]["s_rs"] = a
    univ[s]["s_dv"] = 1 - b
...
json.dump(cert, open("cert7_2026-08-28.json", "w"), ensure_ascii=False, indent=1)
