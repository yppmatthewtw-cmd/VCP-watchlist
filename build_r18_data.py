#!/usr/bin/env python3
"""R18 data build: official 9/10 closes + Yahoo 9/11 closes + 7-item certainty + market-cap tiers (series5: official series through 2026-09-10 merged with Yahoo bars to 2026-09-11).

Sources: zyhe16/top-us-stock-tickers daily snapshots (rebuilt into series5.pkl)
...
SRC, LONG, SPLIT_ADJ = d.get("src", {}), d.get("long", {}), d.get("split_adj", {})
print(f"series sources: official+yahoo {sum(1 for v in SRC.values() if v == 'official+yahoo')} | "
      f"yahoo-only {sum(1 for v in SRC.values() if v == 'yahoo')} | official (no 9/4) {sum(1 for v in SRC.values() if v == 'official')} | "
      f"split-adjusted from the Yahoo basis: {len(SPLIT_ADJ)}")
...
for sym, (d0, f) in SPLITS.items():
    if sym in SPLIT_ADJ:                     # merge_yahoo.py already moved it onto the split-adjusted basis
        print(f"split {sym}: already adjusted from the Yahoo basis (x{SPLIT_ADJ[sym]})")
        continue
...
pickle.dump({"cal": CAL, "series": SER}, open(f"{SCRATCH}/series5adj.pkl", "wb"))
...
MY = set()
for f in ("scan_R22_2026-09-11.json", "scan_stage_R15_2026-09-11.json", "scan_PB-R15_2026-09-11.json"):
...
for s in have:
    ...
    cert[s] = {"cert": round(total, 1), "hl_ok": hl_ok,
               ...
               "src": SRC.get(s, "official"),
               # long Yahoo history (from 2025-09-01, split-adjusted basis): real MA200 / 52-week range
               "ma200_y": LONG.get(s, {}).get("ma200"), "ma50_y": LONG.get(s, {}).get("ma50"),
               "hi52_y": LONG.get(s, {}).get("hi52"), "lo52_y": LONG.get(s, {}).get("lo52"),
               "days52_y": LONG.get(s, {}).get("days52"),
               "chg_6m_y": LONG.get(s, {}).get("chg_6m"), "chg_1y_y": LONG.get(s, {}).get("chg_1y")}

json.dump(cert, open("cert7_2026-09-11.json", "w"), ensure_ascii=False, indent=1)
# any NEW split-like cliff since the last build (9/10 -> 9/11)? (Yahoo-aligned series should show none)
for sym in have:
    fi, cs, vs, ff = SER[sym]
