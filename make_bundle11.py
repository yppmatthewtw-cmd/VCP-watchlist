...
CERT = json.load(open("cert7_2026-09-15.json"))
...
def intra_cell(r):
    """R20: no mid-session column — the 9/15 close (Yahoo, cross-checked) is the newest data."""
    return ""
...
<p class="lede">{list_name}清單內{tier_name}（依 9/15 收盤市值），先按級別（{"A → B → E" if list_no != "2" else "2A → 2B → 1→2"}）、再按該清單分數由高至低{cap_note}{miss_note}。
...
<th>催化</th><th class="num sort">收盤<small>9/15 Yahoo</small></th><th class="num sort">距高</th>{head_extra}
...
<s class="old">A</s><b class="upd">B</b>＝等級變動 · <small class="rk up">▲3</small>／<small class="rk dn">▼3</small>＝名次升降 · <small class="day dn">-1.2%</small>＝9/15 當日漲跌</span></div>
...
<div class="urow"><b>9/15 當日</b><span class="utxt">最強：{esc(up) or "–"}｜最弱：{esc(dn) or "–"}</span></div><div class="urow"><b>數據驗證</b><span class="utxt">本版分級規則與 R19 相同，等級變動全部來自價格與財報。<b>R19 以 Yahoo 單一來源發布的 9/14 收盤，已被 9/15 官方中場快照的 price_change 獨立驗證：240 檔、中位偏差 0.000%，僅 GILD（0.56%）超標</b>；本版 9/15 收盤同樣來自 Yahoo，官方序列已延伸至 9/14。</span></div></div>"""
...
<th class="num sort">收盤<small>9/15 Yahoo</small></th><th class="num sort">距高</th><th class="num sort">市值</th>
...
def write_excel(path, cap_defs, rec, rev, model, stamp_txt, basis_txt):
    ...
    def cap_sheet(ws, snaps, tiers, online, extra_cols, pred):
        ...
        prev_items, _ = top50(PREV_ROWS.get(pkey, []), letter_of, online, pred)
        prev_rank = {r["ticker"]: i for i, r in enumerate(prev_items, 1)}
        cols = (["排名", "前版名次", "代號", "名稱", "產業", "等級", "前版等級", "線上", "催化", "收盤", "基準日", "9/15當日%", "低於高點%"]
    ...
    ws4.append(["代號", "名稱", "產業", "交易所", "上升分數", "前版上升分數", "催化", "主因", "VCP", "前版VCP",
                "Weinstein", "前版Wein", "Pre-breakout", "前版PB", "線上數", "收盤", "9/15當日%", "低於高點%", "市值($B)",
                "級距", "確定性總分", "前版確定性"]
...
def main():
    ...
    ap.add_argument("--prev", default="scan_R24_2026-09-15.json,scan_stage_R17_2026-09-15.json,scan_PB-R17_2026-09-15.json",
                    help="previous release's three snapshot files (vcp,stage,pre) for the red diff")
    ap.add_argument("--prev-rank", default="rank_overlay_R19.json")
    ap.add_argument("--prev-cert", default="cert7_R19.json")
    ap.add_argument("--prev-cat", default="catalysts_R19.json")
    args = ap.parse_args()
    ...
    XC = json.load(open("yahoo_crosscheck_R20.json"))
    V4 = json.load(open("validate_r19_0914.json"))
    ...
    IDX = ("FOMC 會議首日，10 年期美債殖利率升至 <b>5.041%</b>（2007 年以來最高），大市連續第二日下滑："
           "道指 −0.63% 收 52,093.11（−328 點）、標普 −0.45% 收 7,585.73、納指 −0.78% 收 25,981.57。"
           "前一日被重錘的 AI 股部分回穩（QCOM +4.2%、COHR／AMD 各約 +2%），油價續升令能源成為唯一明顯造好的板塊。")
    MACRO = (f"宏觀：<b>9/15 收盤（本表分級基準）</b>— {IDX}"
             f"本清單 {len(allmv)} 檔中位數 {statistics.median(allmv):+.2f}%，板塊最弱 {weakest}；最強 {strongest}。"
             f"下一焦點：<b>今晚 FOMC 議息結果</b>（市場預期加息 25bp）。"
             f"<b>數據</b>：上游快照倉庫仍只在美東上午發布中場快照（9/15 為 11:03 ET），9/15 收盤與成交量取自 <b>Yahoo Finance 日線</b>"
             f"（GitHub Actions runner 以 yfinance 拉取），並與官方序列逐日交叉核對"
             f"（{XC['n_symbols']:,} 檔 × 全窗 {XC['n_pairs']:,} 個收盤，中位偏差 {XC['median_pct']:.3f}%）；"
             f"<b>上版（R19）的 9/14 收盤已被 9/15 官方快照獨立驗證：240 檔中位偏差 0.000%，僅 1 檔（GILD 0.56%）偏差 &gt;0.5%</b>")
...
<li><b>9/15 收盤的來源（請務必留意）</b>：上游快照倉庫（zyhe16/top-us-stock-tickers）自 9/2 起只在美東上午發布一次中場快照，
9/15 亦然（15:03 UTC ＝ 11:03 ET），因此<b>9/15 收盤與成交量取自 Yahoo Finance 日線</b>（本倉庫的 GitHub Actions runner 以 yfinance 拉取後提交，
研究容器的 egress proxy 封鎖 Yahoo）。9/14 及之前每一天都與官方序列逐檔逐日交叉核對：{xc_symbols} 檔、{xc_pairs} 個收盤，
中位偏差 {xc_median}%，偏差 &gt;0.5% 的僅 {xc_gt} 個（{xc_gt_pct}%）。</li>
<li><b>上一版 Yahoo 收盤已獲官方驗證（每版例行檢查）</b>：R19 的 9/14 收盤是 Yahoo 單一來源、當時沒有官方數據可對照。
9/15 的中場快照發布後，其 <code>price_change</code> 欄正好還原出<b>官方 9/14 收盤</b>。
兩者逐檔比對：<b>{v4_n} 檔，中位偏差 {v4_med}%，偏差 &gt;0.5% 的 {v4_gt} 檔（{v4_gt_pct}%）</b> ——
唯一偏差 &gt;0.5% 的是 GILD（0.56%）。Yahoo 作為第二來源<b>連續六版通過驗證</b>：R14 的 9/4 為 2 檔 &gt;0.5%、R15 的 9/8 為 0 檔、R16 的 9/9 僅 HUBB（當時已標示為較舊基準）、R17 的 9/10 為 1 檔、R18 的 9/11 為 0 檔、本輪 R19 的 9/14 為 1 檔。本版的 9/15 收盤同樣要等下一個中場快照（預計 9/16）才能作同一驗證。</li>
...
<li><b>官方序列視窗</b>：自 R19 起，官方快照歷史為 <b>2025-12-26 起（約 180 個交易日）</b>（R19 因研究環境重建而重新完整複製上游倉庫，視窗由 114 日延長至 179 日）。本版與 R19 同一視窗，<b>等級變動不含視窗效應</b>。</li>
<li><b>數據基準</b>：本版為<b>完整的 2026-09-15 收盤重掃</b> —— 官方日線序列已延伸至 9/14，
其後一個交易日（9/15）由 Yahoo 補上；33 檔序列有缺口的 ADR／外國股全程使用 Yahoo。市值按 9/15 收盤重算
（上游市值以 9/15 中場價計，已依收盤／中場價比例換算）。1 月／3 月／5 日動能、MA50 自合併序列計算；
...
均已調整；RUSHB 原掃描的 52 週高低點與均線已按 2/3 換算。確定性 7 項量化以延伸至 9/15 的合併序列重算（274/274 檔有完整序列，
49 檔具一底高於一底結構）。</li>
<li><b>紫色標示＝相對前一版的變動</b>（刻意不用紅色，以免與漲跌顏色混淆）：每頁頂部的「本版更新」框列出新進榜／跌出榜／等級升降／
當日最強最弱；表內紫字為數值已變（滑鼠停留顯示前版值）、<b>NEW</b> 為新進榜、劃線舊等級→紫色新等級為等級變動、
▲▼ 為名次升降、價格下方的小字為 9/15 當日漲跌。確定性分項 |Δ|≥3 才標示，以免全表泛紅。</li>
