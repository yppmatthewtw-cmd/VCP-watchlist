#!/usr/bin/env python3
"""R18 snapshot refresh: 2026-09-11 closes (Yahoo, cross-checked against the official series) for the whole universe, with
the classification fixes from the 2026-09-02 critical review.

Sources: cert7_2026-09-11.json (official series, 241 tickers) + the 9/11 HEAD (mid-session) snapshot for market cap / sector only
...
R22 -> R23, stage R15 -> R16, PB R15 -> R16. Old snapshots are kept untouched.
"""
...
for row in csv.DictReader(io.StringIO(blob.lstrip("﻿"))):
    ...
    try:
        META[s] = {"mcap": float(row["market_cap"] or 0), "sector": row["sector"].strip(),
                   ...
                   "pchg_abs": float(row["price_change"]) if row.get("price_change") not in (None, "") else None}
    except ValueError:
        pass
...
def official(t):
    ...
    m = META.get(t)
    if m and m["price"] > 0:
        # HEAD is a 9/4 MID-SESSION snapshot: price - price_change is the official
        # 9/3 close (the same recovery the series extractor uses), so the 33
        # ADR/gap-series rows sit on the same basis as everyone else.
        if m.get("pchg_abs") is not None:
            return round(m["price"] - m["pchg_abs"], 4), "head_close"
        return m["price"], "head"
    return None, None
...
def apply_official(r):
    ...
    r["price"] = px
    c = CERT.get(t, {})
    # 52-week levels: from the Yahoo daily history (2025-09-01 ->, split-adjusted
    # basis) when it covers the year; else lift the old scan values to any close
    # in the official series beyond them
    if c.get("hi52_y") and (c.get("days52_y") or 0) >= 240:
        r["year_high"] = max(c["hi52_y"], px)
        r["year_low"] = min(c["lo52_y"], px)
        r["range_src"] = "yahoo 52w"
    else:
    ...
    # real MA200 and 6-month / 1-year momentum from the same history
    if c.get("ma200_y"):
        r["ma200"] = round(c["ma200_y"], 2); r["ma200_src"] = "yahoo 200d"
    if c.get("chg_6m_y") is not None:
        r["chg_6m"] = c["chg_6m_y"]; r["c6_src"] = "yahoo 126d"
    if c.get("chg_1y_y") is not None:
        r["chg_1y"] = c["chg_1y_y"]; r["c1y_src"] = "yahoo 252d"
    r["px_src"] = c.get("src") or src
    ...
    for k in ("chg_6m", "chg_1y"):
        if r.get(k) is not None and r.get("c6_src" if k == "chg_6m" else "c1y_src") is None:
            r[k] = round(r[k] + delta, 1)
    ...
    if c.get("chg_1d") is not None:
        ...
        r["chg_1d"] = None      # HEAD percent_change is the 9/4 intraday move, not a 9/3 day change
    ...
    if r.get("chg_6m") is None and c.get("chg_full_pct") is not None and not r.get("c6_src"):
    ...
    if ma:
        r["ma50"] = ma
    if m.get("mcap"):
        # HEAD market cap is struck on the 9/4 intraday price; rescale to the close
        r["mcap"] = m["mcap"] * (px / m["price"]) if m.get("price") else m["mcap"]
    elif c.get("mcap"):
        r["mcap"] = c["mcap"]
    ...
    if not r.get("name") and t == "GAP":
        r["name"] = "Gap Inc."
    for k in ("px_0904_intraday", "chg_0904_intraday"):
        r.pop(k, None)
    r["as_of"] = AS_OF if src == "series" else "2026-09-10"   # HEAD fallback = 9/10 close (mid-session 9/11 snapshot)
    r.pop("stale", None)
...
JOBS = [("scan_R22_2026-09-11.json", "scan_R23_2026-09-12.json", "vcp"),
        ("scan_stage_R15_2026-09-11.json", "scan_stage_R16_2026-09-12.json", "stage"),
        ("scan_PB-R15_2026-09-11.json", "scan_PB-R16_2026-09-12.json", "pb")]
